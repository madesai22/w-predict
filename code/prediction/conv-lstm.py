...
# n_samples x n_in+n_out x width x height x depth where output are last in stack
	if img_dir == 'train':
		...
		start_index = 1355-32*4 # this isn't really right but oh well 
		end_index = 1355
	...
	#print(all_data.shape[0],n_samples)
...
# n_samples x n_in+n_out x width x height x depth where output are last in stack
	if img_dir == 'train':
		...
		start_index = 1355-32*6 # this isn't really right but oh well 
		end_index = 1355
	...
	#print(all_data.shape[0],n_samples)
	out_data = np.zeros((n_samples,n_in+n_out,width,height,depth))
	#print(np.arange (0,all_data.shape[0],(n_in+n_out)))
	j = 0
...
def data_generator(out_data, batch_number, n_in, flag = 0):
	...
	while True:
		...
		for j in np.arange(0,(n_samples//batch_size)*batch_size,batch_size):
			...
			counter +=1
		# if flag == 0:
		# 	plt.imshow(out_data[0,0,:,:,:].reshape(76,23))	
		# 	plt.show()
...
# conv_lstm_output_1 = ConvLSTM2D(5, (6,6), padding='same',dropout=0.2)(inp)
# print(conv_lstm_output_1.shape)
# temp = Reshape((76,23,5,1))(conv_lstm_output_1)
# print(temp.shape)
# temp2 = Permute((3,1,2,4),input_shape=(None, 76, 23, 5))(temp)
#temp2 = Reshape(((None, n_input_frames,input_width,input_height,input_depth)))(temp)
#conv_lstm_output_2 = ConvLSTM2D(input_depth,(3,3),padding='same')(temp2)
...
log_dir = "/Users/helenfellow/Documents/cnn_paper/my_python_code/experimental_lstm/log/3-13"
...
#	img_log_dir = "/Users/helenfellow/Documents/cnn_paper/my_python_code/experimental_lstm/log/2-28-surface-no-overlap"
	file_writer = tf.summary.create_file_writer(log_dir)

	pred_data_imp = organize_sequences('pred',n_input_frames,n_output_frames,width,height,input_depth)
	# plt.imshow(pred_data_imp[0,0,:,:,:].reshape(76,23))
	# plt.show()
	...
	for i in range(0,5):
		...
		axes[1,i].axis('off')
	axes[0,5].pcolormesh(out_[0,:,:,:].reshape(76,23),cmap = newcmp,vmin = _min, vmax = _max)
	axes[0,5].set_title("week 6\n(ground truth)",fontsize=7)
	axes[1,5].pcolormesh(pred_to_keep.reshape(76,23),cmap = newcmp,vmin = _min, vmax = _max)
	axes[1,5].set_title("week 6\n(prediction)",fontsize=7)
	cbaxes = fig.add_axes([.92, 0.1, 0.02, 0.4]) 
	cbar = fig.colorbar(p, label='m/day',cax = cbaxes)

	#file_name = 'epoch_'+str(epoch)+".png"
	#path = "/Users/helenfellow/Desktop/test_no_overlap/"
	#plt.savefig(path+file_name)
	in_out = plot_to_image(fig)
	
	with file_writer.as_default():
		#tf.summary.image("info", tensor_out,max_outputs=2, step = epoch)
		tf.summary.image("in/out", in_out, step = epoch)
		#tf.summary.image("ground truth", out_)
...
a = model.fit_generator(data_generator(train_data,batch_size,n_input_frames),
                    ...
                    epochs=num_epochs, callbacks=[tensorboard_callback,cm_callback],#,myCallback()],#,beholder_callback],#callbacks=[
